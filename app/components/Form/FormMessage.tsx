  ...
  if (errorMessage.length > 1) {
    errorElement = (
      <ul className={clsx('text-small', as == 'text' && 'text-error mt-0-5')}>
        {errorMessage.map((item, idx) => <li className="display-flex" key={`error-${idx}`}><span className="mr-0-5">•</span> {item.charAt(0).toUpperCase() + item.slice(1).replace(/_/g, ' ')}</li>)}
      </ul>
    );
  } else {
    errorElement = errorMessage.map(item => <p key={`error-${item}`} className={clsx('text-small', as == 'text' && 'text-error mt-0-5')}>{item.charAt(0).toUpperCase() + item.slice(1).replace(/_/g, ' ')}</p>)
  }
